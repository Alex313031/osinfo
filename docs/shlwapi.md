...
Starting with version 4.71, the Shell DLLs, among others, began exporting [**DllGetVersion**](https://msdn.microsoft.com/en-us/library/bb776404%28v=vs.85%29). This function can be called by an application to determine which DLL version is present on the system.

**Note** DLLs do not necessarily export [**DllGetVersion**](https://msdn.microsoft.com/en-us/library/bb776404%28v=vs.85%29). Always test for it before attempting to use it.

For Windows versions earlier than Windows 2000, [**DllGetVersion**](https://msdn.microsoft.com/en-us/library/bb776404%28v=vs.85%29) returns a [**DLLVERSIONINFO**](https://msdn.microsoft.com/en-us/library/bb773263%28v=vs.85%29) structure that contains the major and minor version numbers, the build number, and a platform ID. For Windows 2000 and later systems, **DllGetVersion** might instead return a [**DLLVERSIONINFO2**](https://msdn.microsoft.com/en-us/library/bb773265%28v=vs.85%29) structure. In addition to the information provided through **DLLVERSIONINFO**, **DLLVERSIONINFO2**also provides the hotfix number that identifies the latest installed service pack, which provides a more robust way to compare version numbers. Because the first member of **DLLVERSIONINFO2** is a **DLLVERSIONINFO** structure, the later structure is backward-compatible.

### Using the DllGetVersion function

The following sample function `GetVersion` loads a specified DLL and attempts to call its [**DllGetVersion**](https://msdn.microsoft.com/en-us/library/bb776404%28v=vs.85%29) function. If successful, it uses a macro to pack the major and minor version numbers from the [**DLLVERSIONINFO**](https://msdn.microsoft.com/en-us/library/bb773263%28v=vs.85%29) structure into a **DWORD** that is returned to the calling application. If the DLL does not export **DllGetVersion**, the function returns zero. With Windows 2000 and later systems, you can modify the function to handle the possibility that **DllGetVersion** returns a [**DLLVERSIONINFO2**](https://msdn.microsoft.com/en-us/library/bb773265%28v=vs.85%29) structure. If so, use the information in that **DLLVERSIONINFO2** structure's **ullVersion** member to compare versions, build numbers, and service pack releases. The [**MAKEDLLVERULL**](https://msdn.microsoft.com/en-us/library/bb774312%28v=vs.85%29) macro simplifies the task of comparing these values to those in **ullVersion**.

**Note** Using [**LoadLibrary**](https://msdn.microsoft.com/en-us/library/ms684175%28v=vs.85%29) incorrectly can pose security risks. Refer to the **LoadLibrary** documentation for information on how to correctly load DLLs with different versions of Windows.
...
To ensure that your application is compatible with different targeted versions of a .dll file, version macros are present in the header files. These macros are used to define, exclude, or redefine certain definitions for different versions of the DLL. See [Using the Windows Headers](https://msdn.microsoft.com/en-us/library/aa383745%28v=vs.85%29) for an in-depth description of these macros.
